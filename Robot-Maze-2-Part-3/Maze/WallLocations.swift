...
extension ControlCenter {
    ...
    func checkWalls(robot:ComplexRobotObject) -> (up: Bool, right: Bool, down: Bool, left: Bool, numberOfWalls: Int) {
        ...
        print(Bool(isWallUp), Bool(isWallRight), Bool(isWallBottom), Bool(isWallLeft), numberOfWalls)
        return (Bool(isWallUp), Bool(isWallRight), Bool(isWallBottom), Bool(isWallLeft), numberOfWalls)
    }
}